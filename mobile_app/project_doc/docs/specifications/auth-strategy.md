...
For Flutter/mobile clients, use JSON endpoints instead of LiveView form routes:

| Endpoint           | Method | Auth | Description |
| ------------------ | ------ | ---- | ----------- |
| `/api/auth/login`  | POST   | No   | Creates authenticated session cookie |
| `/api/auth/me`     | GET    | Yes  | Returns current authenticated user |
| `/api/auth/logout` | DELETE | Yes  | Invalidates current session |

Request shape for login:
