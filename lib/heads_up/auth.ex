defmodule HeadsUp.Auth do
  ...
  alias HeadsUp.{Users, Auth.UserToken, Auth.UserNotifier}
  ...
  """
  def deliver_user_update_email_instructions(%Users{} = user, current_email, update_email_url_fun)
  ...
  @doc """
  Gets the user by the given API token (Base64url-encoded).
  """
  def get_user_by_api_token(encoded_token) do
    with {:ok, token} <- Base.url_decode64(encoded_token, padding: false),
         {:ok, query} <- UserToken.verify_api_token_query(token) do
      Repo.one(query)
    else
      _ -> nil
    end
  ...
  """
  def deliver_user_reset_password_instructions(%Users{} = user, reset_password_url_fun)
