defmodule HeadsUpWeb.Api.UserAPITest do
  ...
  describe "Friend Access to Goals" do
    test "friends can see friends_only goals", %{conn: conn, friends_only_user: goal_owner} do
      friend = HeadsUp.AuthFixtures.user_fixture()

      # Create friendship
      {:ok, _} = Accounts.send_friend_request(friend.id, goal_owner.id)
      {:ok, _} = Accounts.accept_friend_request(goal_owner.id, friend.id)

      # Create a friends_only goal
      group =
        Repo.insert!(%HeadsUp.Group{
          name: "Test Category",
          description: "Test",
          image_path: "/test.jpg",
          status: :published
      ...
      # Friend should be able to access the goal
      conn =
        conn
        |> log_in_user(friend)
      ...
      assert response["data"]["privacy"] == "friends"
    end

    test "non-friends cannot see friends_only goals", %{conn: conn, friends_only_user: goal_owner} do
      non_friend = HeadsUp.AuthFixtures.user_fixture()
      ...
      # Non-friend should not be able to access the goal
      conn =
        conn
        |> log_in_user(non_friend)
        |> get("/api/goals/#{goal.id}")

      assert json_response(conn, 403)["error"]["message"] == "Access denied"
    end

    test "friends still cannot see private goals", %{conn: conn, private_user: goal_owner} do
      friend = HeadsUp.AuthFixtures.user_fixture()
      ...
      {:ok, goal} =
        HeadsUp.Goals.create_goal(%{
          title: "Private Goal",
          description: "Nobody can see this except owner",
          privacy: :private,
          user_id: goal_owner.id,
          group_id: group.id,
          target_date: DateTime.add(DateTime.utc_now(), 30, :day)
      ...
      # Friend should not be able to access private goal
      conn =
        conn
        |> log_in_user(friend)
        |> get("/api/goals/#{goal.id}")
